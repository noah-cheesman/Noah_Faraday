...
for i =1:length(sols)
    ...
    [ires,freq_vec,Spec,t_sample]=perfect(sol,omega);
    ...
    writetable(array2table([ires',t_sample'],'VariableNames',{'ires','t_sample'}),[currDate,'/datires_',num2str(i),'.txt']);
end

for i =1:length(sols)
    sol=sols{i};
    [omega,omega_s]=omegas(pars,i);
    [freq_vec,Spec,Z,cfreq,x,t_sample]=perfect_x(sol,omega);
    freq_vec=freq_vec/omega;
    freq=omega_s;
    freq2=freq_vec/freq;
    Spec=Spec(freq2<5);
    ...
    writetable(array2table([x',t_sample'],'VariableNames',{'freqs','specs'}),[currDate,'/datxt_',num2str(i),'.txt']);
end
